...
function get_group_avatar_url($gid)
{
	return DATA_HOST ."/groups/". $gid ."/avatar/s_avatar.jpg";
}
...
function write_content_to_file_with_gid($tuid, $content) 
{
	// 生成目录
	$path = DATA_PATH ."/group_". $tuid ."/queue/";
	$file = "";
	$succ = FALSE;
...
{
	try {
		$sql = "INSERT INTO sc_queue (mid, tag_type, gid, fuid, fnick, tuid, queue_type, queue_file, queue_size, image_wh, expire) VALUES ";
		$sql .= "('{$mid}', '{$tag_type}', 0, {$fuid}, '{$fnick}', {$tuid}, '{$queue_type}', '{$queue_file}', {$queue_size}, '{$image_wh}', 0)";
		$stmt = $db->prepare($sql);
		if (!$stmt->execute()) {
...
function write_queue_to_db_for_group($db, $members, $mid, $tag_type, $fuid, $fnick, $gid, $queue_type, $queue_file, $queue_size, $image_wh)
{
	try {
		// 获取所有群组成员
		$pids = array();
		foreach ($members as $member) {
			$sql = "INSERT INTO sc_queue (mid, tag_type, gid, fuid, fnick, tuid, queue_type, queue_file, queue_size, image_wh, expire) VALUES ";
			$sql .= "('{$mid}', '{$tag_type}', {$gid}, {$fuid}, '{$fnick}', {$member}, '{$queue_type}', '{$queue_file}', {$queue_size}, '{$image_wh}', 0)";
...
// return bool
function send_notice_with_apns($qid, $fuser_type, $fuid, $fnick, $tuid, $tag_type)
{
	// 获取收件人的ios_token
	$ios_token = '';
	try {
		...
		// 发送apns
		$apn_res = '';
		$apn_cmd = APNS_URI." '{$ios_token}' '{$qid}' '{$fuser_type}' '{$fuid}' '{$fnick}' '{$tag_type}' '{$tuid}' ";
...
function send_notice_with_socket($tpid, $mqid, $fuser_type, $fuid, $fnick, $tuid, $tag_type)
{
	try {
		...
		// send cmd
		$wbuf = "SENDNOTICETOOTHERPROGRESS pid:{$tpid} mqid:{$mqid} fuser_type:{$fuser_type} fuid:{$fuid} fnick:{$fnick} mtype:{$tag_type} tuid:{$tuid} \r\n\r\n";
		log_info("wbuf:{$wbuf}");
