...
int send_msg_to_another_process_v2(int client_fd, char *pid, char *mqid, char *fuser_type, char *fuid, char *fnick, char *type, char *tuid)
{
	int buf_size = 21 + 4 + strlen(pid) + 6 + strlen(mqid) + 7 + strlen(fuser_type) + 12 + strlen(fuid) + 7 + strlen(fnick) + 7 + strlen(type) + 6 + strlen(tuid) + 12 + 128;
	char *pbuf = (char *)calloc(1, buf_size);
	if (pbuf != NULL) {
		int n = snprintf(pbuf, buf_size, "type:SSYSTEM_SENDMSG pid:%s mqid:%s fuser_type:%s fuid:%s fnick:%s mtype:%s tuid:%s \n", pid, mqid, fuser_type, fuid, fnick, type, tuid);
		send_to_socket(client_fd, pbuf, n);
		log_debug("%s", pbuf);
...
int send_socket_cmd_v2(int client_fd, int client_type, char *qid, char *fuser_type, char *fuid, char *fnick, char *type, char *tuid)
{
	/*char b64_nick[MAX_LINE] = {0};
	int b64_n = base64_decode(fnick, b64_nick, MAX_LINE);
	if (b64_n <= 0) {
	...
	int notice_size = strlen(type) + strlen(qid) + strlen(fuser_type) + strlen(fuid) + strlen(fnick) + strlen(tuid) + b64_size + 512; 
	char *pnotice = (char *)calloc(1, notice_size);
	if (pnotice != NULL) {
		n = snprintf(pnotice, notice_size, "MESSAGENOTICE %s %s %s %s %s %s %s %s", type, qid, fuser_type, fuid, fnick, tuid, pnotice_msg, DATA_END);
